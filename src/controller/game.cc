...
Game::Game(std::istream &in, bool graphics)
    : gameBoard{nullptr}, display{nullptr}, gameInProgress{false}, in{in}
{
  gameBoard = make_shared<GameBoard>();
  if (graphics) display = make_shared<GraphicsDisplay>();
  else display = make_shared<TextDisplay>();
...
void Game::processCmd()
{
  ...
  else if (cmd.front() == "game")
  {
    ...
    for (int i = 0; i < NUMOFPLAYERS; i++)
    {
      ...
      {
        players[i] = make_shared<Computer1>(gameBoard->getEyes());
      }
      else if (cmd[1 + i] == "computer2")
      {
        players[i] = make_shared<Computer2>(gameBoard->getEyes());
      }
      else if (cmd[1 + i] == "computer3")
      {
        players[i] = make_shared<Computer3>(gameBoard->getEyes());
      }
      else if (cmd[1 + i] == "computer4")
      {
        players[i] = make_shared<Computer4>(gameBoard->getEyes());
      }
      else
      {
...
void Game::activate()
{
  while (true)
  {
    try
    {
      ...
      {
        display->display();
        cout << "Enter command for player " << gameBoard->getThisTurn() << endl;
      ...
      {
        cout << "Enter command for setup" << endl;
        setupCmd();
      }
      else
      {
        cout << "Enter command for general" << endl;
        processCmd();
      }
    }
